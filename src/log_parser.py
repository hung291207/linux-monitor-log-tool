...
FAILED_PASSWORD_PATTERN = re.compile(
    r"Failed password for (?:invalid user )?(?P<username>\S+) from (?P<ip_address>[\d.:a-fA-F]+)"
)
...
def _parse_log_line(line: str) -> dict[str, str] | None:
    ...
    parsed["timestamp_iso"] = _parse_timestamp_to_iso(parsed["timestamp"])
    return parsed
...
def _build_base_event(parsed_line: dict[str, str], event_type: str) -> dict[str, str | None]:
    return {
        ...
        "timestamp_iso": parsed_line["timestamp_iso"],
        "host": parsed_line["host"],
        "service": parsed_line["service"],
        "raw_message": parsed_line["message"],
    }
    
def _extract_failed_login_event(parsed_line: dict[str, str]) -> dict[str, str | None]:
    ...
    if failed_password_match:
        ...
        event["source_ip"] = failed_password_match.group("ip_address")
        return event
    ...
    if auth_failure_match:
        ...
        username = auth_failure_match.group("username")
...
def _extract_sudo_event(parsed_line: dict[str, str]) -> dict[str, str | None]:
...
def _extract_ssh_event(parsed_line: dict[str, str]) -> dict[str, str | None]:
...
def parse_auth_log(log_path: Path = AUTH_LOG_PATH, max_events_per_type: int = 50,) -> dict[str, object]:
